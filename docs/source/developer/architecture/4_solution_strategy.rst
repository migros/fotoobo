...
- **Helpers** provide some commonly used functionality throughout fotoobo like result handling of
  the tools or configuration handling (folder :code:`fotoobo/helpers`).
...
API fotoobo should use other information sources like SNMP to collect the data from.
...
To build fotoobo we used the following frameworks.
...
- `requests <https://typer.tiangolo.com/>`_ to interact with the REST APIs.
...
- `Jinja2 <https://pyyaml.org/wiki/PyYAML>`_ for rendering templates.
...
- `Isort <https://www.sphinx-doc.org/en/master/>`_ for consistent import sorting.
- `Pylint <https://black.readthedocs.io/en/stable/>`_ for static code analysis.
- `MyPy <https://www.mypy-lang.org/>`_ for static type checking.
- `pytest <https://www.mypy-lang.org/>`_ for driving and running all the code tests.
