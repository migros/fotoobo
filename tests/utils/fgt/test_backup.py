...
def test_backup_no_backup_dir(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.delattr("fotoobo.utils.fgt.backup_.config.backup_dir")
    monkeypatch.setattr("fotoobo.utils.fgt.backup_.create_dir", MagicMock(return_value=True))
...
def test_backup_all(monkeypatch: MonkeyPatch, temp_dir: str) -> None:
    ...
    monkeypatch.setattr("fotoobo.utils.fgt.backup_.config.backup_dir", temp_dir)
    monkeypatch.setattr("fotoobo.utils.fgt.backup_.create_dir", MagicMock(return_value=True))
    monkeypatch.setattr(
        "fotoobo.utils.fgt.backup_.config.inventory_file", "tests/data/inventory.yaml"
    )
    backup("")
    assert True
...
def test_backup_single(monkeypatch: MonkeyPatch, temp_dir: str) -> None:
    ...
    monkeypatch.setattr("fotoobo.utils.fgt.backup_.os.path.isfile", MagicMock(return_value=True))
    monkeypatch.setattr("fotoobo.utils.fgt.backup_.os.remove", MagicMock(return_value=True))
    backup("test_fgt_1")
    assert True
